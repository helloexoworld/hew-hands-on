...
The FOREACH function can be used to implement it. Complete the skeleton below were the value 0 is the first element pushed in the stack. In WarpScript the FOREACH functions expect a MACRO on top of the stack and a list of element. To start a macro use the keyword "<%" and to end it use the keyword "%>". 
...
The LMAP function will on the other side compute an operation on each elements of a List.
...
Now that you understand the theory about the WarpScript loops, let's see in practice how we can use them to process all the stars in the [next step](https://github.com/helloexoworld/hands-on/tree/master/step-5%20-%20Process%20all%20stars/5.2%20-%20Optimize%20global%20analysis).